...
Работа в Photoshop - Timsi\n\n
ХУДОЖНИК ЦГ И СПРАЙТОВ - PETER KORS\n\n
ОТБЛАГОДАРИТЕ ЕГО ДОНАТОМ ПОЖАЛУЙСТА, ССЫЛКА В ОПИСАНИИ\n\n
Благодарности:\n\n
chupik'у за подбор музыки.\n\n
poi, Mystiss, 140 kilograms of sex и всей беседе «Закодированные» за помощь с кодом.\n\n
Были использованы материалы других модов.\n\n
Авторам также выражаю благодарность."""
...
#Текст-изображения
    ...
    image bottomtext = Text("Скрыть текст",style="rvp",size=40,yalign=.5)
    #Текст реплик вне текстбокса
...
#Изображения c matrixcolor
    image bg_ext_internat_rvp = ConditionSwitch("persistent.sprite_time=='sunset'",im.MatrixColor(ImageReference("bg ext_internat_rvp"), im.matrix.tint(0.94, 0.82, 1.0)),
                                                "persistent.sprite_time=='night'",im.MatrixColor(ImageReference("bg ext_internat_rvp"), im.matrix.tint(0.63, 0.78, 0.82)),
                                                True,ImageReference("bg ext_internat_rvp"))
...
#Кнопки интерфейса
    image right_bar = Transform("gui left_bar_rvp", rotate_pad = False, rotate=180, align=(1.,1.))
    image bottom_bar = Transform("gui left_bar_rvp", rotate_pad = False, yzoom=1.78, rotate=270, align=(0.,1.))
...
screen menu_rvp:
    ...
    imagemap:
        ...
        hotspot((0, 0, 960, 1080)):
            ...
            action [Hide("a_rvp", transition=Dissolve(0.5)),Show("side_a_rvp")]
...
screen side_a_rvp:
    ...
    imagemap:
        ...
        hotspot((0, 0, 960, 1080)):
            ...
            action [Hide("a1_rvp", transition=Dissolve(0.5)),Jump("a1_rvp")]
        hotspot((960, 0, 1920, 1000)):
            hovered [Show("a2_rvp", transition=Dissolve(0.5))]
            unhovered [Hide("a2_rvp", transition=Dissolve(1.0))]
            action [Hide("a2_rvp", transition=Dissolve(0.5)),Jump("a2_rvp")]
screen a1_rvp:
    add "bg walking_rvp":
#надо цг, но для части 1А пока нет
        align(0.,0.)
        crop (480,0,960,1080)
...
screen side_b_rvp:
    ...
    modal False
    imagemap:
        ground Transform("bg_ext_internat_rvp", alpha=0.1)
        hotspot((0, 0, 960, 1080)):
...
#Функция паузы с нотифаем
#сделать новый кэкран
...
#Функция реплик вне текстбокса
label screentext_rvp(replika,posx,posy,xanch):
    show screentext replika zorder 2:
        anchor(xanch,.5) pos(posx,posy)
    with dissolve
    $ renpy.pause()
...
#Функция титров
label credits_rvp(titri):
    show credits titri:
        xalign 0.5
        ypos 1.3
        linear 52.0 ypos -4.0

#Плавный выход в меню мода (by CyberPatsan)
label exit_rvp:
    $ renpy.pause()
...
label a1_rvp:
    ...
    window hide
#1757-бг: при перерисовке заменить номер на 1664
#Я думаю тут вообще абсолют синему сделать: Семён идёт сначала вдоль дома, затем вдоль морд троллейбусов
...
#1787-бг:убрать троллейбус, заменить на многоэтажки
    ...
    window show
    "Однажды Лена предложила бросить свои занятия спортом и тоже найти хоть какую-то подработку после учебы, но я был против." with dissolve
...
#побольше диалогов
    ...
    window hide
#2027-цг:Семён лежит как Гослинг/попугай Кеша
    ...
    show un_rvp normal coat at center:
...
#2112-бг:заменить двор на погружение под воду
    ...
    un "То есть, ты хотел провести вечер с женой, но, пока ждал, решил ещё и накатить с мужиками? И с теми успеть, и со мной?!" with dissolve
...
#накатить режиссуры: Лена крупным планом стоит, видно её от лица вахтёрши фон депо или панелек
    ...
    window show
    "Сегодня мы впервые спали на расстоянии друг от друга." with dissolve
    ...
    "Отвечала она коротко и односложно и я не мог понять тему разговора из услышанного." with dissolve
    ...
    scene bg bedroom_rvp with dissolve
#интерактив: выбор между включить телик, почитать книгу, пописать конспект, подумать над переходом из комнаты в комнату
...
#функция-рука показывает кассету
    "Руки сами нашли кассету." with dissolve
    show kasseta_rvp:
        align(0.5, 0.5) ypos 1.4 zoom .5
        ease 2 ypos .8
    "Кино. Последний герой. Последняя песня альбома." with dissolve
    show kasseta_rvp:
        align(0.5, 0.5) ypos .8 zoom .5
        ease 1 ypos 1.4
    play sound open_cassette_rvp
    play sound open_mafon_rvp
    "Я вставил кассету в магнитофон и включил перемотку." with dissolve
    "Перед тем как включить песню, немного отвлёкся." with dissolve
    ...
    window hide
    show un smile sport:
        ease 2 pos(.5,.5) rotate 0 zoom 1
    window show
    ...
    show un smile sport with dspr
    un "Для неё эти тридцать три квадрата были концом, для нас — станут началом." with dissolve
    ...
    show prologue_dream
    with fade
    call credits_rvp(rvp_credits_a1)
    call exit_rvp
...
#ДОБАВИТЬ ЕБУЧУЮ ПЕСНЮ ИЗ ЛМР - почитать ЛМР
    show dv_rvp far:
        anchor(.5,.5) pos(.3,.7) zoom 1.7
    ...
    $ persistent.sprite_time = "day"
    $ day_time
    "Алиса с Леной зашли в освещённое изнутри здание вокзала."
...
#Алиса смотрела испытывающе. Видать она эту тему и вспомнила.
    ...
    scene cg un_dv pic1_rvp with dissolve
    ...
    show cg un_dv pic2_rvp as cg2 with dspr
    ...
    scene bg_ext_internat_rvp with dissolve
    ...
    show dv guilty pioneer2 with dspr
#подумать над тем, может быть Семён сам пойдёт на вокзал
...
#анимацию спрайта сделать, проверить как это сделано в фиалковых слёзах
    ...
    window hide
    scene bg_ext_internat_rvp with dissolve
    show un_rvp smile sportpioneer at left with dissolve
    window show
    un "Подожди здесь, я сейчас." with dissolve
    show un_rvp smile sportpioneer:
        ease 1 xpos .8 alpha 0
...
#цг: Семен держит Алису за руку как Кетрин в лмр - почитать ЛМР
    show un_rvp shy sportpioneer behind dv:
    ...
    show un_rvp smile3 sportpioneer with dspr
    ...
    show un_rvp normal sportpioneer with dspr
    show dv angry pioneer2 with dspr
    ...
    show un_rvp angry sportpioneer  with dspr
    ...
    show un_rvp grin sportpioneer with dspr
...
#над концом ещё подумать, пока no idea
    ...
    with dissolve
    play ambience ambience_camp_center_day fadein 1
    show un_rvp smile sportpioneer at right with dissolve
...
#думал добавить маску потемнения по краям, пассивно ищу 
    scene bg_ext_internat_rvp:
        ...
        ease .05 offset(-5,0)
        repeat
    show un_rvp smile sportpioneer at right:
        blur 5
        ease .05 offset(0,0)
        ease .05 offset(5,0)
        ease .05 offset(0,0)
    ...
    show un_rvp smile sportpioneer at right:
        ...
        ease .05 offset(-5,0)
        repeat
    dv "Мдаа, нашла ты себе жениха, Лен, ничего не скажешь."
    scene bg_ext_internat_rvp:
        blur 5
    show dv grin pioneer2 at left
    show un_rvp smile sportpioneer at right
    ...
    show un_rvp laugh sportpioneer at right
    ...
    "Спустя полчаса я действительно увидел громаду завода."
    show semen_back_rvp:
    camera:
        linear 2.5 zoom 1.
    "Перед отделом кадров я остановился. А документы? Я открыл мешок с вещами. Там я нашёл конверт, которого раньше не было."
    ...
    "Хотя бы Ф.И.О. не изменили, на том спасибо." with dissolve 
    ...
    show mh_rvp with dissolve
#а почему Семён в галстуке, а Лена с Алисой нет - обыграть в конце, отсылка в 1А на это!
...
#развить диалог, например, что Лена начала заниматься спортом
    ...
    show un smile2 sport close with dissolve
    un "Спокойной ночи!" with dissolve
    window hide
    show un grin sport close at left: #оказывается at left это только xpos, а остальное надо дописать
        anchor(.5,.5) ypos .5
        ease 1 zoom 1.75 pos(.1,.7) rotate -10
        ease 1 zoom 1.25 pos(.33,.5) rotate 0
    window show
    ...
    scene bg square_lmr_night_rvp with dissolve
    $ renpy.pause()
    call credits_rvp(rvp_credits_b1)
    stop music fadeout 2
    ...
    window hide
    show centertext "Продолжение следует..." with dissolve
    call exit_rvp
...
#цгшка ну тут вообще сцену на перепись
...
#заменить текст на изображение формулы, добавить ещё формул?
    ...
    show un_rvp normal pioneer2 with dspr
    un "Да никто намеренно его не срезал."
    ...
    show cg un_roof pic2_rvp as cg2 with dspr
    ...
    "В моём мире были трехмерные модели персонажей компьютерных игр, которые можно было редактировать. Если бы Лена была моделью, то точно над ней поработал кто-то озабоченный."
    un "Или же работой кисти талантливого, но похотливого художника."
...
#расписать, подумать над визуалом, добавить игру!!! БЛПИ Июнь, июль, август| хз насчёт игры, там он же не вратарь
    ...
    scene bg black with dissolve
    $ renpy.pause(1.0)
    show centertext "6 лет назад" with dissolve
    $ renpy.pause(2.0)
...
#добавить Алису со спины
    ...
    hide un_rvp
    show un_rvp rage pioneer2 at left
    with dspr
    ...
    hide un_rvp
    show un_rvp serious pioneer2 at left
    with dspr
    ...
    $ renpy.pause(4.0)
    call credits_rvp(rvp_credits_b2)
    call exit_rvp
...
label a2_rvp:
    ...
    stop music fadeout 1
    pause .5
    play music vse_v_poryadke_rvp fadein 3
    ...
    call screentext_rvp("Скоро наступил праздник. Праздник, из-за которого Лена оставила меня тогда наедине со своими мыслями.",.9,.6,1.)
    call screentext_rvp("7 ноября. День Октябрьской Революции.",.9,.6,1.)
    call screentext_rvp("В детстве остались лишь очень старые воспоминания, как я смотрел на парад. И вот всё пошло по-новой.",.9,.6,1.)
    call screentext_rvp("В тот день я поймал ощущение, будто снова проживаю то время, но во взрослом состоянии.",.9,.6,1.)
    ...
    call screentext_rvp("Лена увлечённо смотрела парад и праздничные передачи. Даже как-то подозрительно молчала, будто ещё и думала о чём-то своём.",.9,.6,1.)
#подумать над тем, чтобы сделать всё через отдельный screen с обводкой, а не label
    call screentext_rvp("{sc=3}{=rvp}{color=#000000}Я даже не видел её лица в тот момент.{/color}{/=rvp}{/sc}",.9,.6,1.)
    call screentext_rvp("Мы не обсуждали это. Я просто ловил момент и смотрел, как страна постепенно уходит в историю.",.9,.6,1.)
    call screentext_rvp("Мысли ворочались в голове.",.9,.6,1.)
    call screentext_rvp("Скоро девяностые. Нужно было что-то делать. Но я не знаю. Я погрузился в спокойную жизнь, в бытовуху. Так жить приятнее, в этой жизни есть Лена.",.9,.6,1.)
    call screentext_rvp("Нужен был запасной план. Но у меня нет знакомых, которые будут строить бизнес. Я – простой водитель.",.9,.6,1.)
    call screentext_rvp("Мысли увязали и захлёбывались, не приводя к какому-то итогу.",.9,.6,1.)
    call screentext_rvp("Хрен с ним, подумаю потом. Не завтра же страна развалится. Ещё годик простоит.",.9,.6,1.)
    stop music fadeout 1
    window hide
    pause 1
    scene bg black with dissolve
...
#таблицу сделать?
    ...
    pause 1.0
    scene bg black with dissolve
#грузить нанобанану или Тимси, чтобы сделать визуал из артов ВадимЗы, убрать галстук и т.д.
#    pause 1
#    scene bg bedroom_rvp with dissolve
...
#    pause 1.0
    ...
    "Я аккуратно развернул Лену. Сигарета уже дотлевала."
    stop music fadeout 1
    show black with dissolve
    pause 1.0
    ...
    with dissolve
    play music pachka_sigaret_rvp fadein 1
    "Лена смотрела в окно. Её лицо было красное от слёз."
    call screentext_rvp("Где сигареты?",.6,.45,0.)
    "Лена молча достала из пальто пачку и отдала мне. Союз-Аполлон. Отец такие любил. Ладно хоть не драла горло каким-нибудь “беломором”."
    "Открыл – полпачки ещё осталось."
    call screentext_rvp("Давно куришь?",.6,.45,0.)
    call screentext_rvp("Ну… нет.\n Первая пачка… вообще.",.4,.57,1.)
    call screentext_rvp("Ладно.",.6,.45,0.)
    call screentext_rvp("А я думаю, чего бюджет \nу нас не сходится.",.6,.45,0.)
    hide cg balkon un_1_rvp as un1
    show cg balkon un_2_rvp as un2 zorder 3
    with dissolve
    "Лена закрыла лицо руками. Она сгорала от стыда."
    call screentext_rvp("Да ладно, Лен, не казнись. \nМне не жалко.",.6,.45,0.)
    hide cg balkon me_1_rvp as me1
    show cg balkon me_2_rvp as me2 zorder 3
    with dissolve
    "Я взял одну, затем спички на раме. И ещё одну протянул Лене."
    call screentext_rvp("Будешь?",.6,.45,0.)
    "Лена непонимающе смотрела и ждала."
    call screentext_rvp("Тебе нервы унять надо.",.6,.45,0.)
    "Лена молча взяла сигарету. Я зажёг спичку и прикурил нам обоим."
    hide cg balkon un_2_rvp as un2
    show cg balkon un_1_rvp as un1 zorder 3
    with dissolve
    "Мы оба повернулись к окну и начали смотреть вдаль."
    call screentext_rvp("Ну, Лен, рассказывай.",.6,.45,0.)
    call screentext_rvp("Ты же не просто так начала \nэто.",.6,.45,0.)
    call screentext_rvp("Устала… от всего этого.",.4,.57,1.)
    call screentext_rvp("Я, наверное, со стороны \nтакая, «железная леди».",.4,.57,1.)
    call screentext_rvp("Но у меня тоже есть \nпредел прочности.",.4,.57,1.)
    call screentext_rvp("Скоро сессия. \nПервая сессия в этом институте.",.4,.57,1.)
    call screentext_rvp("Не знаю, как \nона у меня пройдёт.",.4,.57,1.)
    call screentext_rvp("Нужно всё на пятёрки сдать,\n иначе слечу с “повышки”.",.4,.57,1.)
#Повышенная стипендия для отличников - добавить в справочник
    call screentext_rvp("А значит меньше денег\n у нас в бюджете.",.4,.57,1.)
    call screentext_rvp("Деньги должен я приносить.",.6,.45,0.)
    hide cg balkon un_1_rvp as un1
    show cg balkon un_3_rvp as un3 zorder 3
    with dissolve
    call screentext_rvp("Сём, я понимаю. Но я не хочу, чтобы\n ты калымил или шабашил где-то.",.4,.57,1.)
    call screentext_rvp("Вот сейчас ты на верном пути,\n я считаю. Работаешь, учишься.",.4,.57,1.)
    call screentext_rvp("Перейдёшь на работу прямо \nпо твоей специальности.",.4,.57,1.)
    call screentext_rvp("Закончишь институт, \nдадут инженера.",.4,.57,1.)
    "Ох, Лен, через год-два всё так поменяется. Да уже сейчас, если подумать."
    call screentext_rvp("Да, не всё сразу.\n Зато у тебя будет образование, \nстаж по специальности.",.4,.57,1.)
    call screentext_rvp("И что мне этот стаж, \nна хлеб намазать?",.6,.45,0.)
    call screentext_rvp("Это значит ты опытнее будешь \nи лучше работать. Начальство \nэто заметит и будет премии платить.",.4,.57,1.)
    call screentext_rvp("Если ты, конечно, \nлоботрясничать не будешь.",.4,.57,1.)
    "Лена осеклась."
    hide cg balkon un_3_rvp as un3
    show cg balkon un_1_rvp as un1 zorder 3
    hide cg balkon me_2_rvp as me2
    show cg balkon me_1_rvp as me1 zorder 3
    with dissolve
    call screentext_rvp("Извини, что опять \nнотации читаю.",.4,.57,1.)
    call screentext_rvp("Так. Что-то ещё случилось?",.6,.45,0.)
    call screentext_rvp("Ну, ещё та ситуация с тобой.\n Я не хочу тебя ни в чём обвинять, \nно это выбило меня из колеи.",.4,.57,1.)
    call screentext_rvp("И ты решила взяться за меня,\n ставя себя в роль воспитателя.\n И это нагрузило тебя\n дополнительной ответственностью.",.6,.45,0.)
    call screentext_rvp("Да, повоспитывать \nтебя немного.",.4,.57,1.)
    hide cg balkon un_1_rvp as un1
    show cg balkon un_3_rvp as un3 zorder 3
    call screentext_rvp("Сёма, ты уж извини.\n Но из нас двоих я \nболее дисциплинированная.",.4,.57,1.)
    "Вижу, Лен, как твоя дисциплина меж пальцев дымится."
    call screentext_rvp("Я пыталась тянуть нас\n обоих. Ты видишь, к\n чему это привело.",.4,.57,1.)
    "Ладно, не время язвить и спорить. Лена так старается, что уже надрывается."
    call screentext_rvp("Что ж, признаю вину.",.6,.45,0.)
    call screentext_rvp("А с людьми как? \nНикакая тварь \nтебя не обижает?",.6,.45,0.)
    "Вспомнилась фраза из фильма. Пафосно, но пойдёт."
    call screentext_rvp("Преподы, одногруппники? \nКомсомол опять голову морочит?",.6,.45,0.)
    call screentext_rvp("Нет, тут всё гладко.\n У комсомола вообще вопросов\n нет – я в сборной института играю,\n отличница. Помогаю, если\n нужно что-то срочно нарисовать.",.4,.57,1.)
    call screentext_rvp("Лена, у тебя такая нагрузка.",.6,.45,0.)
    call screentext_rvp("Сёма, ты даже не представляешь,\n насколько мне важно, чтобы ты был\n со мной. Терпел меня и мои заскоки.",.4,.57,1.)
    call screentext_rvp("Я без тебя просто развалюсь.",.4,.57,1.)
    hide cg balkon un_3_rvp as un3
    show cg balkon un_1_rvp as un1 zorder 3
    call screentext_rvp("Я это поняла, когда ты \nушёл после нашей ссоры. \nЛадно хоть вещи не собрал, \nпоняла, что не навсегда.",.4,.57,1.)
    call screentext_rvp("Ты такой спокойный сейчас.\n А я тогда так тебя облаяла. \nСтыдно и вспомнить.",.4,.57,1.)
    call screentext_rvp("Тебя можно понять. Там \nбыла ситуация более экстремальная.",.6,.45,0.)
    call screentext_rvp("То есть?",.4,.57,1.)
    call screentext_rvp("Ну, ты же после курения не \nдерёшься с кем-то чёрт знает где?",.6,.45,0.)
    call screentext_rvp("Хи-хи, ну да. Ну это только\n вы, мужики, так делаете.",.4,.57,1.)
    "Ну да, Лен. Ты-то у нас морды никому не била."
    call screentext_rvp("Мы с тобой друг друга стоим. \nУходим, скрываем \nвредные привычки.",.6,.45,0.)
    call screentext_rvp("Да уж. Два сапога пара.",.4,.57,1.)
    "Мы с Леной немного помолчали."
    call screentext_rvp("А почему ты на меня не кричишь?\n Будто тебе немного\n безразлично то, что я делаю.",.4,.57,1.)
    call screentext_rvp("Вовсе не безразлично.\n Но я не вижу в крике смысла.",.6,.45,0.)
    call screentext_rvp("Если с тобой это происходит,\n значит на то были причины.",.6,.45,0.)
    call screentext_rvp("И за три года вместе мы\n так и не перестали скрывать.",.4,.57,1.)
    call screentext_rvp("Что ж, бывает и так. \nНам надо работать над этим.",.6,.45,0.)
    call screentext_rvp("Часто в семьях могут \nдесятками лет скрывать \nчто-то друг от друга. \nВещи пострашнее курения.",.6,.45,0.)
    "Лена понимающе промолчала."
    call screentext_rvp("А как ты скрывала запах изо рта?",.6,.45,0.)
    call screentext_rvp("Ну… я ещё жвачку жевала.",.4,.57,1.)
    call screentext_rvp("А, классика.",.6,.45,0.)
    "Лена достала пачку жвачки."
    call screentext_rvp("Хочешь?",.4,.57,1.)
    call screentext_rvp("Не откажусь. А то чего мне вонять.",.6,.45,0.)
    call screentext_rvp("А у нас была жвачка “Love is”. \nПереводится как “Любовь это…”",.6,.45,0.)
    call screentext_rvp("Хорошее название. \nИ что, по-твоему, любовь?",.4,.57,1.)
    "Я немного помолчал и ответил."
    call screentext_rvp("Любовь это… желать любимому\n человеку только самое лучшее…\n и помогать, чем можешь.",.6,.45,0.)
    "Вот так я подвёл итог."
    call screentext_rvp("Хорошие слова.",.4,.57,1.)
    window hide
    pause 1.0
    scene bg black with dissolve
    stop music fadeout 1
    ...
    show un smile sport:
        ...
        ease 1.5 pos(.5,.5) alpha 1
    window show
    "Когда мы возвращались с балкона, я вспомнил."
    me " Надо бы тебя наказать всё-таки."
    show un shy sport with dspr
    "Лена смутилась."
    un "Ну… раз надо."
    me "Итак. За мелкое воровство из семейного бюджета, утаивание о своём плохом моральном состоянии и нанесение вреда своему организму Елена Персунова приговаривается к…"
    me "...подробным докладам о своём состоянии своему мужу и к влажной уборке квартиры."
    show un smile3 sport with dspr
    "Лена широко улыбнулась."
    un "Ух, как строго!"
    un "Знаешь, Сём."
    un "Давай ты будешь рулить нашей семьёй. У тебя своя «метода» и она не хуже моей."
    un "Ты такой молодец, что спокойно меня выслушал. Я себя теперь чувствую гораздо лучше."
    pause 1.0
    scene bg black with dissolve
    ...
    show un smile sport with dissolve:
        align(.85,.5) xzoom -1
    "Я разделся, помыл руки и зашёл. Там меня ждал… торт? Да ещё и со свечой."
    "День рождения? У Лены недавно был в сентябре."
    "Я поднял глаза на Лену. Она торжественно улыбалась. Отблески свечи играли в её глазах."
    show un smile2 sport with dspr
    un "Сёма, с днём рождения!"
    me "В смысле?"
    un "Ну как же, твой первый день рождения!"
    me "А, ну да!"
    show un smile sport with dspr
    "Мой день рождения. В паспорте от Генды в этой графе стоял день прибытия в Совёнок. И я решил праздновать мой день рождения в этом мире именно так."
    "Но Лена помнила настоящую дату, я ей когда-то говорил. Помнила все эти годы и решила поздравить."
    me "Почему решила отметить?"
    un "Просто… мне кажется сейчас это очень нужно тебе. Не забывать себя здесь… в этом мире."
    me "Спасибо, Лен. Мне, правда, очень приятно."
    window hide
    show un smile sport:
        align(.85,.5)
        ease 2 xpos .9
        ease 2 zoom 2.5 pos(1.2,.8)
    window show
    "Я подошёл к Лене, обнял и поцеловал её. Мы постояли несколько секунд."
    un "Не забудь загадать желание и задуть свечу."
    window hide
    pause 1.0
    scene bg black with dissolve
    ...
    pause 1.0
    play ambience ambience_cold_wind_loop fadein 1
    play music vse_v_poryadke_rvp fadein 1
    scene okno_night_zoom_rvp with dissolve
    $ set_mode_rvp(nvl)
    window show
    ...
    "Вот уже три с половиной года, как я в этом мире. Я начал вспоминать: Совёнок, райцентр. А теперь и город Горький."
    "Но ведь я жил своей жизнью в конце нулевых. А теперь тут, в конце восьмидесятых."
    "Вот ведь они блаженны в своём неведении – не знают, что произойдёт через год-два. Особенно Лена."
    "Надо бы подумать, что делать. Бизнес делали… да на всём чём угодно. Главное найти знакомого-энтузиаста и… надёжную “крышу”."
    nvl clear
    "Да, бизнесмен ты наш будущий, погибнуть не боишься? Лену оставить одну?"
    "Но этот страх не отвратил меня от идеи. Да, нужно найти тех, кто будет организовывать бизнес."
    "Только где? В депо таких нет. Там люди живут от зарплаты до зарплаты и их всё устраивает."
    "Одногруппники Лены? Я их не знаю."
    nvl clear
    "Да и чем я могу быть полезен? Могу чинить, водить машину. В общем всё, чтобы быть шестёркой у воротил и получать крошки со стола."
    "Мда, вот тебе и бизнес, Семён Семёныч."
    "Пока, пожалуй, поучусь. Может хоть научусь чему и буду полезен."
    "Лена всё-таки права, хоть и в неожиданной ипостаси. Знания – сила."
    "Этот год надо доучиться. А вот следующим летом – что-то пробовать."
    "А сейчас – смотреть, кто вокруг меня. Заводить полезные знакомства, но не за рюмкой водки в гараже, а в более приличных местах."
    pause 1.0
    "На этом я завершил своё планирование. Довольный этим и собой, что всё-таки обсудил с собой давно наболевший вопрос, я решил повспоминать прошлое."
    "Я не родился ещё не в России, но в ней вырос."
    "Интересно, там время идёт также как и тут? Тогда я там уже четвёртый год как без вести пропавший."
    "А может, время там остановилось или течёт гораздо медленнее?"
    "А есть ли там Лена? Хотя почему она там должна быть?"
    "Вопрос не давал мне покоя. Почему-то мне явно казалось, что в моём мире можно встретить не только Лену, но и Алису и Славю."
    "Как будто… я их уже встречал."
    window hide
    $ set_mode_rvp()
    show un normal sport:
        align(.5,.5) pos(-.2,.5) alpha 0
        ease 1.5 pos(.5,.5) alpha 1
    "На балкон снова, как в тот раз, зашла Лена."
    show un normal sport with dspr
    un "Чего не спим?"
    me "Думаю."
    un "И как, много надумал?"
    me "Кое-что надумал. Но не всё."
    me "Разобраться бы с моим попаданием сюда."
    show un smile sport with dspr
    un "Давай подумаем вместе."
    me "Да мне вот интересно, есть ли ты в моём прошлом мире?"
    show un angry2 sport with dspr
    un "А что мне там делать? Я же тут."
    me "А что если ты тоже попала в Совёнок, сев на автобус?"
    un "Ну… да. Мы все приехали в лагерь из райцентра на автобусе."
    me "Нет, я имею в виду… что ты попала из того же мира, что и я."
    un "Сёма, это уже какой-то бред. Я родилась и выросла в Лениноморске. Помню всю свою жизнь там."
    me "Ну… возможно ты переродилась и живёшь здесь."
    show un serious sport with dspr
    un "Сёма, хватит дурить!"
    me "А, может, это всё сон, который длится уже четвёртый год."
    show un surprise sport with dspr
    me "И ты нереальна, а я проснусь у себя в однушке без тебя."
    ...
    window show
    "Ну куда?!"
    "Лена убежала из балкона. Чёрт, возможно она даже обиделась."
    "Или расстроилась. Я ей наговорил, что жду, когда встречу другую Лену."
    ...
    "Я помолчал немного."
    me "Просто, хочу чтоб ты знала."
    me "Неважно, во сне я или нет. После всех этих дней я понял – мне хорошо, когда со мной есть ты."
    ...
    play ambience ambience_cold_wind_loop fadein 1
    call credits_rvp(rvp_credits_a2)
    call exit_rvp