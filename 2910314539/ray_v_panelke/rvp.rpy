python:
    ...
    """
    #пусть эта ебучая напоминалка будет тут:
    #   репозиторий клонируется по такому пути - D:\SteamLibrary\steamapps\workshop\content\331470\ray_v_panelke\2910314539\ray_v_panelke
    #Чтобы мод заработал нужно удалить первую папку ray_v_panelke, чтобы после 331470 был сразу 2910314539.
...
label a2_rvp:
    ...
    me "Давно куришь?"
    un "Ну… нет. Первая пачка… вообще."
    me "Ладно."
    me "А я думаю, чего бюджет у нас не сходится."
    "Лена закрыла лицо руками. Она сгорала от стыда."
    me "Да ладно, Лен, не казнись. Мне не жалко."
    "Я взял одну, затем спички на раме. И ещё одну протянул Лене."
    me "Будешь?"
    "Лена непонимающе смотрела и ждала."
    me "Тебе нервы унять надо."
    "Лена молча взяла сигарету. Я зажёг спичку и прикурил нам обоим."
    "Мы оба повернулись к окну и начали смотреть вдаль."
    me "Ну, Лен, рассказывай."
    me "Ты же не просто так начала это."
    un "Устала… от всего этого."
    un "Я, наверное, со стороны такая, «железная леди»."
    un "Но у меня тоже есть предел прочности."
    un "Скоро сессия. Первая сессия в этом институте. Не знаю, как она у меня пройдёт."
    un "Нужно всё на пятёрки сдать, иначе слечу с “повышки”."
    un "А значит меньше денег у нас в бюджете."
    me "Деньги должен я приносить."
    un "Сём, я понимаю. Но я не хочу, чтобы ты калымил или шабашил где-то."
    un "Вот сейчас ты на верном пути, я считаю. Работаешь, учишься. Перейдёшь на работу прямо по твоей специальности. Закончишь институт, дадут инженера."
    "Ох, Лен, через год-два всё так поменяется. Да уже сейчас, если подумать."
    un "Да, не всё сразу. Зато у тебя будет образование, стаж по специальности."
    me "И что мне этот стаж, на хлеб намазать?"
    un "Это значит ты опытнее будешь и лучше работать. Начальство это заметит и будет премии платить. Если ты, конечно, лоботрясничать не будешь."
    "Лена осеклась."
    un "Извини, что опять нотации читаю."
    me "Так. Что-то ещё случилось?"
    un "Ну, ещё та ситуация с тобой. Я не хочу тебя ни в чём обвинять, но это выбило меня из колеи."
    me "И ты решила взяться за меня, ставя себя в роль воспитателя. И это нагрузило тебя дополнительной ответственностью."
    un "Да, повоспитывать тебя немного."
    un "Сёма, ты уж извини. Но из нас двоих я более дисциплинированная."
    "Вижу, Лен, как твоя дисциплина меж пальцев дымится."
    un "Я пыталась тянуть нас обоих. Ты видишь, к чему это привело."
    "Ладно, не время язвить и спорить. Лена так старается, что уже надрывается."
    me "Что ж, признаю вину."
    me "А с людьми как? Никакая тварь тебя не обижает?"
    "Вспомнилась фраза из фильма. Пафосно, но пойдёт."
    me " Преподы, одногруппники? Комсомол опять голову морочит?"
    un "Нет, тут всё гладко. У комсомола вообще вопросов нет – я в сборной института играю, отличница. Помогаю, если нужно что-то срочно нарисовать."
    me "Лена, у тебя такая нагрузка."
    un "Сёма, ты даже не представляешь, насколько мне важно, чтобы ты был со мной. Терпел меня и мои заскоки."
    un "Я без тебя просто развалюсь."
    un "Я это поняла, когда ты ушёл после нашей ссоры. Ладно хоть вещи не собрал, поняла, что не навсегда."
    un "Ты такой спокойный сейчас. А я тогда так тебя облаяла. Стыдно и вспомнить."
    me "Тебя можно понять. Там была ситуация более экстремальная."
    un "То есть?"
    me "Ну, ты же после курения не дерёшься с кем-то чёрт знает где?"
    un "Хи-хи, ну да. Ну это только вы, мужики, так делаете."
    "Ну да, Лен. Ты-то у нас морды никому не била."
    me "Мы с тобой друг друга стоим. Уходим, скрываем вредные привычки."
    un "Да уж. Два сапога пара."
    "Мы с Леной немного помолчали."
    un "А почему ты на меня не кричишь? Будто тебе немного безразлично то, что я делаю."
    me "Вовсе не безразлично. Но я не вижу в крике смысла."
    me "Если с тобой это происходит, значит на то были причины."
    un "И за три года вместе мы так и не перестали скрывать."
    me "Что ж, бывает и так. Нам надо работать над этим."
    me "Часто в семьях могут десятками лет скрывать что-то друг от друга. Вещи пострашнее курения."
    "Лена понимающе промолчала."
    me "А как ты скрывала запах изо рта?"
    un "Ну… я ещё жвачку жевала."
    me "А, классика."
    "Лена достала пачку жвачки."
    un "Хочешь?"
    me "Не откажусь. А то чего мне вонять."
    me "А у нас была жвачка “Love is”. Переводится как “Любовь это…”"
    un "Хорошее название. И что, по-твоему, любовь?"
    "Я немного помолчал и ответил."
    me "Любовь это… желать любимому человеку только самое лучшее… и помогать, чем можешь."
