...
#sementext и untext должны быть заменены
label sementext_rvp(replika):
    show screentext_left replika zorder 2:
        anchor(0.,.5) pos(.6,.45)
    with dissolve
    $ renpy.pause()
    hide screentext_left with dissolve
    return
label untext_rvp(replika):
    show screentext_right replika zorder 2:
        anchor(1.,.5) pos(.4,.57)
    with dissolve
    $ renpy.pause()
...
#переписать авторов реплик так что бы первый мил. был добрым, а второй злым
    ...
    window hide
    stop music fadeout 1
#бг подумать над заменой, но режиссурой удалось пофиксить
...
#повтор слова окно
    ...
    call sementext_rvp("Где сигареты?")
    ...
    call sementext_rvp("Давно куришь?")
    call untext_rvp("Ну… нет.\n Первая пачка… вообще.")
    call sementext_rvp("Ладно.")
    call sementext_rvp("А я думаю, чего бюджет \nу нас не сходится.")
    hide cg balkon un_1_rvp as un1
    show cg balkon un_2_rvp as un2 zorder 3
    with dissolve
    "Лена закрыла лицо руками. Она сгорала от стыда."
    call sementext_rvp("Да ладно, Лен, не казнись. \nМне не жалко.")
    ...
    call sementext_rvp("Будешь?")
    "Лена непонимающе смотрела и ждала."
    call sementext_rvp("Тебе нервы унять надо.")
    ...
    call sementext_rvp("Ну, Лен, рассказывай.")
    call sementext_rvp("Ты же не просто так начала \nэто.")
    call untext_rvp("Устала… от всего этого.")
    call untext_rvp("Я, наверное, со стороны \nтакая, «железная леди».")
    call untext_rvp("Но у меня тоже есть \nпредел прочности.")
    call untext_rvp("Скоро сессия. \nПервая сессия в этом институте.")
    call untext_rvp("Не знаю, как \nона у меня пройдёт.")
    call untext_rvp("Нужно всё на пятёрки сдать,\n иначе слечу с “повышки”.")
#Повышенная стипендия для отличников - добавить в справочник
    call untext_rvp("А значит меньше денег\n у нас в бюджете.")
    call sementext_rvp("Деньги должен я приносить.")
    hide cg balkon un_1_rvp as un1
    show cg balkon un_3_rvp as un3 zorder 3
    with dissolve
    call untext_rvp("Сём, я понимаю. Но я не хочу, чтобы\n ты калымил или шабашил где-то.")
    call untext_rvp("Вот сейчас ты на верном пути,\n я считаю. Работаешь, учишься.")
    call untext_rvp("Перейдёшь на работу прямо \nпо твоей специальности.")
    call untext_rvp("Закончишь институт, \nдадут инженера.")
    "Ох, Лен, через год-два всё так поменяется. Да уже сейчас, если подумать."
    call untext_rvp("Да, не всё сразу.\n Зато у тебя будет образование, \nстаж по специальности.")
    call sementext_rvp("И что мне этот стаж, \nна хлеб намазать?")
    call untext_rvp("Это значит ты опытнее будешь \nи лучше работать. Начальство \nэто заметит и будет премии платить.")
    call untext_rvp("Если ты, конечно, \nлоботрясничать не будешь.")
    "Лена осеклась."
    ...
    show cg balkon me_1_rvp as me1 zorder 3
    with dissolve
    call untext_rvp("Извини, что опять \nнотации читаю.")
    call sementext_rvp("Так. Что-то ещё случилось?")
    call untext_rvp("Ну, ещё та ситуация с тобой.\n Я не хочу тебя ни в чём обвинять, \nно это выбило меня из колеи.")
    call sementext_rvp("И ты решила взяться за меня,\n ставя себя в роль воспитателя.\n И это нагрузило тебя\n дополнительной ответственностью.")
    call untext_rvp("Да, повоспитывать \nтебя немного.")
    hide cg balkon un_1_rvp as un1
    show cg balkon un_3_rvp as un3 zorder 3
    call untext_rvp("Сёма, ты уж извини.\n Но из нас двоих я \nболее дисциплинированная.")
    "Вижу, Лен, как твоя дисциплина меж пальцев дымится."
    call untext_rvp("Я пыталась тянуть нас\n обоих. Ты видишь, к\n чему это привело.")
    "Ладно, не время язвить и спорить. Лена так старается, что уже надрывается."
    call sementext_rvp("Что ж, признаю вину.")
    call sementext_rvp("А с людьми как? \nНикакая тварь \nтебя не обижает?")
    "Вспомнилась фраза из фильма. Пафосно, но пойдёт."
    call sementext_rvp("Преподы, одногруппники? \nКомсомол опять голову морочит?")
    call untext_rvp("Нет, тут всё гладко.\n У комсомола вообще вопросов\n нет – я в сборной института играю,\n отличница. Помогаю, если\n нужно что-то срочно нарисовать.")
    call sementext_rvp("Лена, у тебя такая нагрузка.")
    call untext_rvp("Сёма, ты даже не представляешь,\n насколько мне важно, чтобы ты был\n со мной. Терпел меня и мои заскоки.")
    call untext_rvp("Я без тебя просто развалюсь.")
    hide cg balkon un_3_rvp as un3
    show cg balkon un_1_rvp as un1 zorder 3
    call untext_rvp("Я это поняла, когда ты \nушёл после нашей ссоры. \nЛадно хоть вещи не собрал, \nпоняла, что не навсегда.")
    call untext_rvp("Ты такой спокойный сейчас.\n А я тогда так тебя облаяла. \nСтыдно и вспомнить.")
    call sementext_rvp("Тебя можно понять. Там \nбыла ситуация более экстремальная.")
    call untext_rvp("То есть?")
    call sementext_rvp("Ну, ты же после курения не \nдерёшься с кем-то чёрт знает где?")
    call untext_rvp("Хи-хи, ну да. Ну это только\n вы, мужики, так делаете.")
    "Ну да, Лен. Ты-то у нас морды никому не била."
    call sementext_rvp("Мы с тобой друг друга стоим. \nУходим, скрываем \nвредные привычки.")
    call untext_rvp("Да уж. Два сапога пара.")
    "Мы с Леной немного помолчали."
    call untext_rvp("А почему ты на меня не кричишь?\n Будто тебе немного\n безразлично то, что я делаю.")
    call sementext_rvp("Вовсе не безразлично.\n Но я не вижу в крике смысла.")
    call sementext_rvp("Если с тобой это происходит,\n значит на то были причины.")
    call untext_rvp("И за три года вместе мы\n так и не перестали скрывать.")
    call sementext_rvp("Что ж, бывает и так. \nНам надо работать над этим.")
    call sementext_rvp("Часто в семьях могут \nдесятками лет скрывать \nчто-то друг от друга. \nВещи пострашнее курения.")
    "Лена понимающе промолчала."
    call sementext_rvp("А как ты скрывала запах изо рта?")
    call untext_rvp("Ну… я ещё жвачку жевала.")
    call sementext_rvp("А, классика.")
    "Лена достала пачку жвачки."
    call untext_rvp("Хочешь?")
    call sementext_rvp("Не откажусь. А то чего мне вонять.")
    call sementext_rvp("А у нас была жвачка “Love is”. \nПереводится как “Любовь это…”")
    call untext_rvp("Хорошее название. \nИ что, по-твоему, любовь?")
    "Я немного помолчал и ответил."
    call sementext_rvp("Любовь это… желать любимому\n человеку только самое лучшее…\n и помогать, чем можешь.")
    "Вот так я подвёл итог."
    call untext_rvp("Хорошие слова.")
    window hide
    pause 1.0
    scene bg black with dissolve
    stop music fadeout 1
